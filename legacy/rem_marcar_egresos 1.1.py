...
#   lista de keywords -> "SI" si alguna aparece (normalizado, sin tildes)
#   "_no_vacio"       -> "SI" si la celda no está vacía ni es un negativo
#   "_no_chileno"     -> "SI" si hay nacionalidad y NO contiene "CHILE"
DEMOGRAFIA = {
    "Gestante":            (["ALERTAS"],              ["GESTANTE", "EMBARAZ"]),
    "Madre_menor5":        (["USTED", "MADRE"],       ["SI"]),
    "Pueblos_Originarios": (["PUEBLO", "ORIGINARIO"], "_no_vacio"),
    "SENAME":              (["ALERTAS"],              ["SENAME", "REINSERCION"]),
    "Proteccion_Ninez":    (["ALERTAS"],              ["MEJOR NINEZ", "PROTECCION ESPECIAL"]),
    "Migrante":            (["NACIONALIDAD"],         "_no_chileno"),
}
...
def procesar(entrada: Path, salida: Path):
    ...
    print(f"[corte] modo={modo} | encabezado en fila {header_idx} (la hoja original NO se modifica)")
    ...
    print(f"[A05] RUT=col{rut_col} | Edad(llenado)=col{edad_col} | Sexo=col{sexo_col}")
    ...
    print("[demo] " + " ".join(f"{f}=col{c}" for f, (c, _) in demo_cols.items()) +
    ...
    wb.save(salida)
    print(f"[ok] guardado: {salida}  (hoja '{NOMBRE_HOJA_SALIDA}')")
    n = {}
    for e in eventos:
