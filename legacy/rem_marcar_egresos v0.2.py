...
def procesar(entrada: Path, salida: Path):
    ...
    cols = ["RUT", "Edad_Formulario", "Sexo", "Tipo_Egreso", "Patologia", "Subtipo", "Fila_Origen"]
    ws2.append(cols)
    orden = {"Alta": 0, "Traslado": 1, "OtrasCausas": 2}
    eventos.sort(key=lambda e: (orden.get(e["tipo"], 9), str(e["pat"]), str(e["sub"])))
    for e in eventos:
        ws2.append([e["rut"], e["edad"], e["sexo"], TIPO_LABEL.get(e["tipo"], e["tipo"]),
                    e["pat"], e["sub"], e["fila"]])
    ...
    for col, w in zip("ABCDEFG", (14, 8, 8, 14, 46, 28, 11)):
