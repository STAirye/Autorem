...
| Archivo | Rol |
|---|---|
| `rem_utils.py` | **Base genérica.** `norm`, `buscar_col`, `encontrar_fila_encabezado` (parametrizado), `ArchivoInvalido`, `VERSION`, `grid`, `rut_valido`/`dv_rut`, lectura pandas (`leer_xlsx`, robusto a la «dimension» rota; `resolver_columnas`; `contiene_todos`/`contiene_alguno`), `marcar_demografia`, `cargar_maestro`/`maestro_rem_map` y **`filtrar_mes`** (§3.1). |
